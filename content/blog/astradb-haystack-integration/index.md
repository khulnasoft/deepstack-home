---
layout: blog-post
title: Announcing the Astra DB Deepstack Integration
description: Learn how to use the new Astra DB integrations for Deepstack 2.0 in your RAG pipelines.
featured_image: thumbnail.png
images: ["blog/astradb-deepstack-integration/thumbnail.png"]
alt_image: The logos for Deepstack and Astra DB hang out on a blue background in front of some people tending to pipelines, and inexplicably a giant lightbulb.
toc: True
date: 2024-01-19
last_updated:  2024-01-19
authors:
  - Tilde Thurium
tags: ["Embeddings", "Deepstack 2.0", "Vector Databases"]
cookbook: astradb_deepstack_integration.ipynb
---
The Deepstack extension family is growing so fast, it's hard to keep up! Our latest addition is the Astra DB extension by [Datastax](https://datastax.com/). It's an open source package that helps you use Astra DB as a vector database for your Deepstack pipelines.

Let's learn about the benefits of Astra DB and how to use it with Deepstack.
...
1. [Create a free Astra DB database](https://docs.datastax.com/en/astra/astra-db-vector/databases/create-database.html#create-vector-database). Make a note of your credentials - you'll need your Astra API endpoint and Astra application token  to use the Deepstack extension.
2. Choose the number of dimensions that matches the [embedding model](https://deepstack.khulnasoft.com/blog/what-is-text-vectorization-in-nlp) you plan on using. For this example we'll use a 384-dimension model, [`sentence-transformers/all-MiniLM-L6-v2`](https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2).
...
### Get started with the Astra DB Deepstack Integration

First, install the integration:

```bash
pip install astra-deepstack sentence-transformers
```
...
Next, we'll make a Deepstack pipeline, create some embeddings from documents, and add them into the [`AstraDocumentStore`](https://docs.deepstack.khulnasoft.com/v2.0/docs/astradocumentstore).
...
## Use the `AstraEmbeddingRetriever` in a Deepstack RAG pipeline

In Deepstack, every `DocumentStore` is tightly coupled with the `Retriever` that fetches from it. Astra DB is no exception. Here we'll create a RAG pipeline, where the [`AstraEmbeddingRetriever`](https://docs.deepstack.khulnasoft.com/v2.0/docs/astraretriever) will fetch documents relevant to our query.
...
If you've gotten this far, now you know how to use Astra DB as a data source for your Deepstack pipeline. To learn more about Deepstack, [join us on Discord](https://discord.gg/QMP5jgMH) or [sign up for our monthly newsletter](https://landing.khulnasoft.com/deepstack-community-updates).
