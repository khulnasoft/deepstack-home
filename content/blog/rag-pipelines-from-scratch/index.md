...
description: "Let's build a simple RAG Pipeline with Deepstack 2.0 by just connecting three components: a Retriever, a PromptBuilder and a Generator."
...
authors:
  - Sara Zanzottera
tags: ["Deepstack 2.0", "RAG"]
canonical_url: https://www.zansara.dev/posts/2023-10-27-deepstack-series-rag/
---	

Retrieval Augmented Generation (RAG) is quickly becoming an essential technique to make LLMs more reliable and effective at answering any question, regardless of how specific. To stay relevant in today's NLP landscape, Deepstack must enable it.

Let's see how to build such applications with Deepstack 2.0, from a direct call to an LLM to a fully-fledged, production-ready RAG pipeline that scales. At the end of this post, we will have an application that can answer questions about world countries based on data stored in a private database. At that point, the knowledge of the LLM will be only limited by the content of our data store, and all of this can be accomplished without fine-tuning language models.

> 💡 *I recently gave a talk about RAG applications in Deepstack 2.0, so if you prefer videos to blog posts, you can find the recording [here](https://zansara.dev/talks/2023-10-12-office-hours-rag-pipelines/). Keep in mind that the code might be slightly outdated.*
...
Let's build one of these with Deepstack 2.0!
...
> ⚠️ **Warning:** *This code was tested on `deepstack-ai==2.0.0b5`. Deepstack 2.0 is still unstable, so later versions might introduce breaking changes without notice until Deepstack 2.0 is officially released. The concepts and components however stay the same.*

## Generators: Deepstack's LLM components

As every NLP framework that deserves its name, Deepstack supports LLMs in different ways. The easiest way to query an LLM in Deepstack 2.0 is through a Generator component: depending on which LLM and how you intend to query it (chat, text completion, etc...), you should pick the appropriate class.

We're going to use `gpt-3.5-turbo` (the model behind ChatGPT) for these examples, so the component we need is [`OpenAIGenerator`](https://docs.deepstack.khulnasoft.com/v2.0/docs/openaigenerator). Here is all the code required to use it to query OpenAI's `gpt-3.5-turbo` :
...
Right now, Deepstack supports HuggingFace models through the [`HuggingFaceLocalGenerator`](https://docs.deepstack.khulnasoft.com/v2.0/docs/huggingfacelocalgenerator) and [`HuggingFaceTGIGenerator`](https://docs.deepstack.khulnasoft.com/v2.0/docs/huggingfacetgigenerator) components, and many more LLMs are coming soon.
...
Deepstack offers a component that can render variables into prompt templates: it's called [`PromptBuilder`](https://docs.deepstack.khulnasoft.com/v2.0/docs/promptbuilder). As the generators we've seen before, also `PromptBuilder` is nearly trivial to initialize and use.
...
Thankfully, retrieving relevant information from large [corpora](https://en.wikipedia.org/wiki/Text_corpus) (a technical term for extensive collections of data, usually text) is a task that Deepstack excels at since its inception: the components that perform this task are called [Retrievers](https://docs.deepstack.khulnasoft.com/v2.0/docs/retrievers).
...
Let's create a small local database to store information about some European countries. Deepstack offers a neat object for these small-scale demos: `InMemoryDocumentStore`. This document store is little more than a Python dictionary under the hood but provides the same exact API as much more powerful data stores and vector stores, such as [Elasticsearch](https://github.com/khulnasoft/deepstack-core-integrations/tree/main/document_stores/elasticsearch) or [ChromaDB](https://deepstack.khulnasoft.com/integrations/chroma-documentstore). Keep in mind that the object is called "Document Store" and not simply "datastore" because what it stores is Deepstack's Document objects: a small dataclass that helps other components make sense of the data that they receive.
...
Once the document store is set up, we can initialize a retriever. In Deepstack 2.0, each document store comes with its own set of highly optimized retrievers: `InMemoryDocumentStore` offers two, one based on BM25 ranking and one based on embedding similarity.
...
`InMemoryDocumentStore` is clearly a toy implementation: Deepstack supports much more performant document stores such as [Elasticsearch](https://deepstack.khulnasoft.com/integrations/elasticsearch-document-store), [ChromaDB](https://deepstack.khulnasoft.com/integrations/chroma-documentstore) and [Marqo](https://deepstack.khulnasoft.com/integrations/marqo-document-store). Since we have built our app with a BM25 retriever, let's select Elasticsearch as our production-ready document store of choice.
...
```python
from elasticsearch_deepstack.document_store import ElasticsearchDocumentStore
...
Now, let's write again our four documents into the store. In this case, we specify the duplicate policy, so if the documents were already present, they would be overwritten. All Deepstack document stores offer three policies to handle duplicates: `FAIL` (the default), `SKIP`, and `OVERWRITE`.

```python
from deepstack.document_stores import DuplicatePolicy
documents = [
    Document(content="German is the the official language of Germany."), 
    Document(content="The capital of France is Paris, and its official language is French."),
    Document(content="Italy recognizes a few official languages, but the most widespread one is Italian."),
...
```python
from elasticsearch_deepstack.bm25_retriever import ElasticsearchBM25Retriever
...
In this post, we've detailed some fundamental components that make RAG applications possible with Deepstack: Generators, the PromptBuilder, and Retrievers. We've seen how they can all be used in isolation and how you can make Pipelines out of them to achieve the same goal. Last, we've experimented with some of the (very early!) features that make Deepstack 2.0 production-ready and easy to scale up from a simple demo with minimal changes.
