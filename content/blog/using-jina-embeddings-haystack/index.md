---
layout: blog-post
title: 'Using Jina Embeddings v2 with Deepstack 2.0 pipelines to summarize legal documents'
description: Learn how to use the Jina v2 Embedding models in a RAG pipeline with our new Deepstack integration.
featured_image: thumbnail.png
images: ["blog/using-jina-embeddings-deepstack/thumbnail.png"]
alt_image: The logos for Deepstack and Jina AI sit against a blue background with cartoonish art of a man and a woman holding a magnifying glass up to a large clipboard, next to a gavel.
date: 2024-01-10
...
With the [Jina Deepstack extension](https://deepstack.khulnasoft.com/integrations/jina), you can now take advantage of these new text embedders in your Deepstack pipelines! In this post, we'll show what's cool about Jina Embeddings v2 and how to use them.

> You can follow along in the accompanying [Colab notebook of a RAG pipeline that uses the Jina Deepstack extension](https://colab.research.google.com/drive/1l8GbQhqxnWXkdktgJfs9Rz4EAtNbHK_L#scrollTo=_coq_qCuItbN).
...
## Getting started using Jina Embeddings v2 with Deepstack

To use the integration you'll need a free Jina api key - get one [here](https://jina.ai/embeddings/). 

You can use Jina Embedding models with two Deepstack components: `JinaTextEmbedder` and `JinaDocumentEmbedder`.

To create semantic embeddings for documents, use [`JinaDocumentEmbedder`](https://docs.deepstack.khulnasoft.com/v2.0/docs/jinadocumentembedder) in your indexing pipeline. For generating embeddings for queries, use [`JinaTextEmbedder`](https://docs.deepstack.khulnasoft.com/v2.0/docs/jinatextembedder). 

In the following code we'll demonstrate how to use both components. You can also [see the Deepstack docs for some minimum viable code examples.](https://deepstack.khulnasoft.com/integrations/jina)


## Summarizing legal text with a Deepstack RAG pipeline
...
```bash
pip install jina-deepstack chroma-deepstack pypdf
```
...
Our indexing pipeline will preprocess the legal document, turn it into vectors, and store them. We'll use the [Chroma DocumentStore](https://docs.trychroma.com/getting-started) to store the vector embeddings, via the [Chroma Document Store Deepstack integration](https://deepstack.khulnasoft.com/integrations/chroma-documentstore).

```python
from chroma_deepstack.document_store import ChromaDocumentStore
document_store = ChromaDocumentStore()
```

At a high level, the [`LinkContentFetcher`](https://docs.deepstack.khulnasoft.com/v2.0/docs/linkcontentfetcher) pulls this document from its URL. Then we convert it from a PDF into a Document object Deepstack can understand.
...
from deepstack.components.preprocessors import DocumentCleaner
from deepstack.components.preprocessors import DocumentSplitter
from chroma_deepstack.retriever import ChromaEmbeddingRetriever
...
In Deepstack 2.0 `retriever`s are tightly coupled to `DocumentStores`. If we pass the document store in the `retriever` we initialized earlier, this pipeline can access those embeddings we generated, and pass them to the LLM.
...
Thanks for reading! If you want to stay on top of the latest Deepstack developments, you can [subscribe to our newsletter](https://landing.khulnasoft.com/deepstack-community-updates) or [join our Discord community](https://discord.com/invite/deepstack).
...
- [What is text vectorization in NLP?](https://deepstack.khulnasoft.com/blog/what-is-text-vectorization-in-nlp)
- [The definitive guide to BERT models](https://deepstack.khulnasoft.com/blog/the-definitive-guide-to-bertmodels)
