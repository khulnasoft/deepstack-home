...
authors:
  - Tuana Celik
canonical_url: https://www.khulnasoft.com/blog/how-to-build-a-semantic-search-engine-in-python
...
Over the last decade or so, Python has become the principal language for machine learning (ML) and natural language processing (NLP). In this article, we will show you how to set up a semantic search engine in Python, placing it on top of your document collection of choice, with our open source  [Deepstack framework](https://deepstack.khulnasoft.com/). Thanks to Deepstack’s modular setup and the availability of high-quality pre-trained language models, you’ll be able to set up your own  [semantic search](https://www.khulnasoft.com/blog/understanding-semantic-search)  system in less than twenty minutes.

## A refresher on semantic search

Like all  [Transformer-based language models](https://www.khulnasoft.com/blog/what-is-a-language-model), the models used in semantic search encode text (both the documents and the query) as high-dimensional vectors or embeddings. We can then use similarity measures like cosine similarity to understand how close in meaning two vectors (and their associated texts) are. Texts that are similar in meaning are closer to each other, while unrelated texts are more distant. While illegible to humans, the  [vector-based representation](https://www.khulnasoft.com/blog/what-is-text-vectorization-in-nlp)  works very well for computers to represent meaning.

The  [superiority of semantic search](https://www.khulnasoft.com/blog/what-semantic-search-can-do-for-you)  over a keyword-based approach becomes clear if we look at an example. Think of the difference between the queries “why can’t I commit changes” — a perennial problem for the novice Git user — and “why can’t I commit to changes” — a problem for the indecisive. The preposition “to” entirely changes the meaning of the query, which is impossible to detect with simple keyword matching. A semantic language model (like the one  [used by Google](https://blog.google/products/search/search-language-understanding-bert/)) will embed the two queries in disparate locations of the vector space.
...
Now, let’s look at how Deepstack and Python make it possible to build semantic search systems in just a few lines of code.

## Deepstack: Applied NLP in Python

Deepstack is our framework for applied NLP that uses a modular, mix-and-match approach to building NLP systems. These days, the highest-performing language models are huge.  [Training such a language model](https://www.khulnasoft.com/blog/when-and-how-to-train-a-language-model)  yourself on your local PC is impractical, uneconomical and, frankly, often impossible. However, thanks to centralized platforms like the  [Hugging Face model hub](https://huggingface.co/models), pre-trained models can be shared and reused by everyone.

With pre-trained models being readily available, the Deepstack framework offers the architecture that makes them work in practice: the Python library comes with several  [preconfigured pipelines](https://docs.deepstack.khulnasoft.com/docs/ready_made_pipelines), where all you need to do is plug in the language model that works for you. This makes it easy to build prototype systems quickly, using different models. At the same time, Deepstack offers all the flexibility that we’ve come to expect from Python libraries, making it extremely easy to configure and customize your own pipeline.
...
## Build Your Semantic Search System With The Deepstack Python Framework

We’ll be working with the  [FAISS](https://github.com/facebookresearch/faiss)  [document store](https://deepstack.khulnasoft.com/components/document-store)  as our database, which is optimized for working with vector representations. Make sure to install Deepstack (we use version 1.11) with FAISS support enabled:

```python
pip install git+https://github.com/khulnasoft/deepstack.git  
pip install 'farm-deepstack[faiss]'
```
You can then start by reading in and converting the files, having stored them as .txt documents locally under  _datapath_. If you want to use the same ._txt_  files that we use in this project, you can download them from  [this Google Drive folder](https://drive.google.com/drive/folders/1-gX1DlmVodP6OVRJC3WBRZoGgxPuJvvt?usp=sharing)  Deepstack’s  _convert_files_to_docs_  function recognizes the files’ format automatically and converts them into objects of the  [Document](https://docs.deepstack.khulnasoft.com/docs/documents_answers_labels#document)  class, which can then be read into the document store.
```python
from deepstack.utils import convert_files_to_docs  
...
```python
from deepstack.document_stores.faiss import FAISSDocumentStore
...
```python
from deepstack.pipelines import DocumentSearchPipeline
...
As an alternative to using a more complex model, you can expand your pipeline by setting up a custom design that contains a  [ranker node](https://deepstack.khulnasoft.com/pipeline_nodes/ranker)  in addition to the retriever. The ranker uses its own Transformer-based language model to re-rank the documents it receives from the retriever.
...
from deepstack.nodes import SentenceTransformersRanker  
...
```
Adding a ranker to your semantic search engine can definitely improve your results, especially if you’re unhappy with the retriever’s results. From here on, there are no limits to your creativity. For instance, the retriever-ranker setup works well in connection with a  [summarizer node](https://docs.deepstack.khulnasoft.com/docs/summarizer), in a popular  [custom pipeline setup](https://docs.deepstack.khulnasoft.com/docs/pipelines#example-creating-a-retriever-ranker-summarizer-pipeline). And, of course, we would be remiss if we didn’t mention the crucial role of semantic search in most applications of open-domain  [question answering](https://www.khulnasoft.com/blog/deepstack-question-answering-at-scale)  (QA).
...
## Come Join the Deepstack Community

Deepstack is the framework of choice for the Python programmer looking to implement a semantic search system. Moreover, anyone who wants to use the latest NLP models in their product — be it an app, a website, or something else — can do so easily through Deepstack’s modular and application-oriented nature.

If you want to tweak your semantic search system and use, say, a different database as your document store, have a look at our  [documentation](https://docs.deepstack.khulnasoft.com/).

Do you have more questions or simply want to see what other people are up to in the applied NLP space? We run a vibrant  [NLP community on Discord](https://deepstack.khulnasoft.com/community), which is open to everyone. Meet other open source enthusiasts and talk directly to our team members.

Want to add more nodes to your pipeline, fine-tune your own models or create new datasets? Why not check out the  [Deepstack repository on GitHub](https://github.com/khulnasoft/deepstack)  — and give us a star while you’re there?
