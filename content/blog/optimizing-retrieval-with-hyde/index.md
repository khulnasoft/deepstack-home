...
description: See how you can easily incorporate HyDE into Deepstack RAG pipelines to optimize retrieval
...
[Hypothetical Document Embeddings (HyDE)](https://docs.deepstack.khulnasoft.com/v2.0/docs/hypothetical-document-embeddings-hyde) is a technique proposed in the paper “[Precise Zero-Shot Dense Retrieval without Relevance Labels](https://aclanthology.org/2023.acl-long.99/)” which improves retrieval by generating “fake” hypothetical documents based on a given query, and then uses those “fake” documents embeddings to retrieve similar documents from the same embedding space.

In this article, we will see how to implement and incorporate it into Deepstack by creating a [custom component](https://docs.deepstack.khulnasoft.com/v2.0/docs/custom-components) that implements HyDE.


> To learn more about how HyDE works, and where it's useful, check out our guide on [Hypothetical Document Embeddings (HyDE)](https://docs.deepstack.khulnasoft.com/v2.0/docs/hypothetical-document-embeddings-hyde) 

## Build a Pipeline to Create Hypothetical Document Embeddings

First, let's build a simple pipeline to generate these hypothetical documents. To do so, we will use the following Deepstack components:

-   [`PromptBuilder`](https://docs.deepstack.khulnasoft.com/v2.0/docs/promptbuilder) and [`OpenAIGenerator`](https://docs.deepstack.khulnasoft.com/v2.0/docs/openaigenerator) to query an instruction-following language model and generate hypothetical documents.
-   [`SentenceTransformersDocumentEmbedder`](https://docs.deepstack.khulnasoft.com/v2.0/docs/sentencetransformersdocumentembedder) encodes the hypothetical documents into vector embeddings.
-  [`OutputAdapter`](https://docs.deepstack.khulnasoft.com/v2.0/docs/outputadapter) to adapt the output of the `Generator` to be compatible with the input of the `SentenceTransformersDocumentEmbedder`, which expects `List[Document]`
...
from deepstack.components.converters import OutputAdapter
...
```python
from deepstack import Pipeline, component, Document, default_to_dict, default_from_dict
from deepstack.components.converters import OutputAdapter
...
If you’ve gotten this far, you know how to use the HyDE technique and how easy it is to incorporate it into Deepstack. To learn more about Deepstack,  [join us on Discord](https://discord.gg/QMP5jgMH)  or  [sign up for our monthly newsletter](https://landing.khulnasoft.com/deepstack-community-updates).
