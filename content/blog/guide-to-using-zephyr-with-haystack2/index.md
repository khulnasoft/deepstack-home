...
description: Learn how to use the new Zephyr models in Deepstack 2.0 pipelines with the upcoming HuggingFaceLocalGenerator
featured_image: thumbnail.png
featured_image_caption: Build retrieval-augmented pipelines using open-source LLMs from Hugging Face 🪁
images: ["blog/guide-to-using-zephyr-with-deepstack2/thumbnail.png"]
toc: True
date: 2023-11-06
...
authors:
  - Stefano Fiorucci
  - Tuana Celik
tags: ["Deepstack 2.0", "LLM", "Open Source"]
...
Following the theme of [our previous article](https://deepstack.khulnasoft.com/blog/customizing-rag-to-summarize-hacker-news-posts-with-deepstack2), we will show you how to build a pipeline that uses Zephyr with Deepstack, but we will also take the opportunity and show you how to do this with the preview package of Deepstack 2.0.
...
## Querying Zephyr with Deepstack

To start with, let’s see how to query [Zephyr-7B-Beta](https://huggingface.co/HuggingFaceH4/zephyr-7b-beta) with Deepstack as it is. To load and manage open-source LLMs in Deepstack 2.0, we can use the `HuggingFaceLocalGenerator`. If you are using a free Colab instance (with limited resources), you can load the model using **4-bit quantization** (passing the appropriate `huggingface_pipeline_kwargs` to our Generator). For an introduction to Quantization in Hugging Face Transformers, you can read [this simple blog post](https://huggingface.co/blog/merve/quantization).
...
Now that we’ve seen the pretty simple process of initializing a Deepstack 2.0 (preview) component that can query Zephyr, let’s have a look at how we can set up a full pipeline that works on our own data. This pipeline will use Zephyr-7B-Beta as the LLM to generate responses.

### The Prompt

The first step is to define a prompt template that will effectively describe the manner in which we interact with an LLM. In Deepstack 2.0 (preview, but eventually also the actual major release), prompt templates can be defined using the Jinja2 templating language. This provides a wide range of customizability to your prompts at query time. These templates are then used by a `PromptBuilder` that builds a full prompt before sending it to an LLM.
...
Let’s quickly walk through what we are seeing above. We will look at both the syntax present to prompt Zephyr specifically, and the syntax that we use in Deepstack 2.0 (preview) to build prompts programmatically.
...
You will also notice that there is a for loop in the prompt. What you are seeing is fully Jinja2 templating language. We use Jinja2 with Deepstack 2.0 prompt templates. You can modify this section depending on what sort of input you want to provide the LLM. 
...
In the next section, we will see how we use this prompt template in a full Deepstack 2.0 (preview) pipeline so that we can fill this template to create a fully formed prompt for Zephyr.
...
To do so, we will be using 4 components and 1 document store. For demonstration purposes, we’ve used the `InMemoryDocumentStore`. However, you could use a number of other document stores such as Weaviate, Pincone, Milvus etc. (see our [document store integrations](https://deepstack.khulnasoft.com/integrations?type=Document+Store) for a full list).
